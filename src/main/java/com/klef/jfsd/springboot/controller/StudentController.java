...
public class StudentController 
{
	...
	@PostMapping("updatestdprofile")
	 ...
	 @GetMapping("/student-portfolios")
	    ...
	    @GetMapping("/mentors")
	    public String viewMentors(Model model) {
	        List<Mentor> mentors = mentorService.getAllMentors(); // Fetch mentors from service
	        model.addAttribute("mentors", mentors); // Add to model for JSP rendering
	        return "viewmentors"; // Return JSP name to render
	    }
	    
	    @GetMapping("/addProject")
	    public String addProjectPage(Model model) {
	        List<Mentor> mentors = mentorService.getAllMentors(); // Fetch all mentors
	        model.addAttribute("mentors", mentors); // Add mentors to the model
	        return "addproject"; // JSP for adding a project
	    }
	     
		/*
		 * @Autowired private JavaMailSender mailSender;
		 * 
		 * @PostMapping("sendemail") public ModelAndView sendEmail(HttpServletRequest
		 * request) throws Exception { String name = request.getParameter("name");
		 * String toemail = request.getParameter("email"); String subject
		 * =request.getParameter("subject"); String msg =
		 ...
		 * 
		 * helper.setTo(toemail); helper.setSubject(subject);
		 * helper.setFrom("ramisettyabhiram.25@gmail.com");
		 * 
		 * String htmlContent = "<h3>Contact Form Details</h3>" +
		 * "<p><strong>Name:</strong> " + name + "</p>" + "<p><strong>Email:</strong> "
		 * + toemail + "</p>" + "<p><strong>Subject:</strong> " + subject + "</p>" +
		 * "<p><strong>Message:</strong> " + msg + "</p>" + "<p><strong>OTP:</strong> "
		 * + otp + "</p>";
		 * 
		 * 
		 * helper.setText(htmlContent, true); mailSender.send(mimeMessage);
		 * 
		 * ModelAndView mv = new ModelAndView("mailsuccess"); mv.addObject("message",
		 ...
		 */
		  ...
		  {
			  ModelAndView mv = new ModelAndView();
			  mv.setViewName("submit-contact");
			  return mv;
		  }
		  
		  @GetMapping("stdcontactus")
			public ModelAndView stdcontactus()
			{
				ModelAndView mv = new ModelAndView();
